...
test -e /usr/lib/jvm/openjdk11 && set -gx JAVA_HOME /usr/lib/jvm/openjdk11
...
abbr g 'git'
abbr gethosts 'sudo curl https://raw.githubusercontent.com/odoochain/hosts/master/hosts -o /etc/hosts'
...
abbr lyrics 'python3 ~/Repos/lyric-get/main_cli.py'
abbr mkdir 'mkdir -pv'
abbr mv 'mv -iv'
...
function udevadd
  set -l id (lsusb | fzf | cut -d ' ' -f6)
  if string match -req '^[0-9a-f]{4}:[0-9a-f]{4}$' "$id"
    set -l vendor (echo $id | cut -d ':' -f1)
    set -l product (echo $id | cut -d ':' -f2)
